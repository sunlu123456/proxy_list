...
class Crawl(object):
    ...
    def __proxy(self):
        ...
        return {'protocol': proxy[0], 'ip': proxy[1], 'port': proxy[2]}

    @config.catch_exception_logging(None)
    def __proxy_delete(self, protocol, ip, port):
        """
        删除一个已爬代理
        :param ip:
        :param port:
        ...
        response = requests.get(
            'http://{}:{}/proxy/delete?protocol={}&ip={}&port={}'.format(config.WEB_API_IP, config.WEB_API_PORT, protocol, ip, port),
            **{'timeout': 5})

        if response.status_code == 204:
            config.console_log('删除代理成功 %s://%s:%s' % (protocol, ip, port), 'green')
        else:
            config.console_log('删除代理失败 %s://%s:%s' % (protocol, ip, port), 'red')
        ...
        if config.REQUEST_PROXY_RETRY and proxy:
            ...
            while text is None:
                ...
                if retried > config.REQUEST_PROXY_RETRY:
                    self.__proxy_delete(proxy['protocol'], proxy['ip'], proxy['port'])
                    proxy = self.__proxy()
                    # 清空代理重试次数
                    retried = 0
                text = self.__request(proxy, url)
                config.console_log('使用代理%s，请求页面 %s ,总尝试第 %s 次' % (proxy, url, try_total), 'white')
        else:
